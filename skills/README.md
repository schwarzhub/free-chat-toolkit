...
The first cluster is an **academic-research workflow suite** — from reading the literature to
shipping a submission-ready paper and its derivatives. Skills compose: `replicate-paper` feeds
`replication-package`; `systematic-review` feeds `draft-paper`; `draft-paper` feeds `adversarial-review`
→ `reviewer-response`. They lean on tools free-chat already has — `run_code` (now with
numpy/pandas/scipy/matplotlib/sympy), `web_search`, `fetch_url`, `ask_model` (multi-model panels) —
...
| 1.3 | [`systematic-review`](systematic-review) ✅ | Review-article synthesis across a corpus (PRISMA-style: search→screen→extract→synthesize) | `scholar_search*`, `fetch_fulltext*`, `ask_model` |
| 1.4 | `literature-inconsistency-probe` ▫ | Find where the literature disagrees (effect signs, magnitudes, definitions) and characterize why | `scholar_search*`, `ask_model` |
...
| 2.6 | `submission-qa` ▫ | Journal-readiness gate: format, word/figure counts, statements, cover letter, suggested reviewers, package | `read_document*`, `journal_specs*` |
...
- `fetch_repo` — read a public code/data repo's tree + files (GitHub/OSF/Zenodo, read-only).
